...
router.get('/:code', async (req, res) => {
    ...
    const countBasket = await Basket.count({where: {shop_id: shop.id, ssid: req.sessionID}})
...
router.get('/:code/basket/', async (req, res) => {
    ...
    const countBasket = await Basket.count({where: {shop_id: shop.id, ssid: req.sessionID}})
    const basket = await Basket.findAll({where: {shop_id: shop.id, ssid: req.sessionID}})
    var ids = [], summPrice = 0
...
router.get('/:code/new-order/', async (req, res) => {
    ...
    const countBasket = await Basket.count({where: {shop_id: shop.id, ssid: req.sessionID}})
    if(countBasket == 0){ return res.redirect('/'+shop.url+'/catalog/')}
    const basket = await Basket.findAll({where: {shop_id: shop.id, ssid: req.sessionID}})
    var ids = [], summPrice = 0
...
router.post('/:code/add_to_basket', async (req, res) => {
    ...
    const haveInBasket = await Basket.findOne({where: {shop_id: shop.id, ssid: req.sessionID, item_id: item_id}})
...
router.post('/:code/catalog-detail/:id', async (req, res) => {
    ...
    const {item_id, name, price, prop} = req.body
    const haveInBasket = await Basket.findOne({where: {shop_id: shop.id, ssid: req.sessionID, item_id: item_id, properties: prop}})
